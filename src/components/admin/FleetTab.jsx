import { useState } from 'react'
import { Pencil, Archive, RotateCcw, X, Bus } from 'lucide-react'
import { useAdmin } from '../../context/AdminContext'

const EMPTY = {
  id: '', busNumber: '', licensePlate: '',
  schoolId: '', driverId: '',
  currentLat: '', currentLng: '',
}

const STATUS_CFG = {
  waiting: 'bg-gray-100 text-gray-600',
  enRoute: 'bg-blue-100 text-blue-700',
  arrived: 'bg-green-100 text-green-700',
}

function ErrorBanner({ message, onDismiss }) {
  return (
    <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 mb-4 flex items-center justify-between text-sm">
      <span>{message}</span>
      <button onClick={onDismiss} className="ml-4 text-red-400 hover:text-red-600"><X size={16} /></button>
    </div>
  )
}

function Field({ label, value, onChange, required, placeholder, children }) {
  return (
    <div>
      <label className="block text-xs font-medium text-[#666666] mb-1">{label}</label>
      {children || (
        <input value={value} onChange={onChange} required={required} placeholder={placeholder}
          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#F98C1F]" />
      )}
    </div>
  )
}

export default function FleetTab() {
  const {
    filteredBuses, drivers, schools, error, clearError,
    handleSaveBus, handleArchiveBus,
  } = useAdmin()

  const [showArchived, setShowArchived] = useState(false)
  const [modalOpen, setModalOpen] = useState(false)
  const [editTarget, setEditTarget] = useState(null)
  const [form, setForm] = useState(EMPTY)
  const [saving, setSaving] = useState(false)

  const visible = filteredBuses.filter(b => showArchived ? b.isArchived : !b.isArchived)
  const activeDrivers = drivers.filter(d => !d.isArchived)
  const activeSchools = schools.filter(s => !s.isArchived)

  function openCreate() {
    setForm(EMPTY)
    setEditTarget(null)
    setModalOpen(true)
  }

  function openEdit(bus) {
    setForm({ ...EMPTY, ...bus })
    setEditTarget(bus)
    setModalOpen(true)
  }

  function closeModal() {
    setModalOpen(false)
    setEditTarget(null)
  }

  const set = field => e => setForm(f => ({ ...f, [field]: e.target.value }))

  async function handleSubmit(e) {
    e.preventDefault()
    setSaving(true)
    try {
      const prevDriverId = editTarget?.driverId || null
      await handleSaveBus(form, prevDriverId)
      closeModal()
    } finally {
      setSaving(false)
    }
  }

  async function toggleArchive(bus) {
    await handleArchiveBus(bus.id, !bus.isArchived)
  }

  const driverName = id => activeDrivers.find(d => d.id === id)?.name || '—'

  return (
    <div className="p-6">
      {error && <ErrorBanner message={error} onDismiss={clearError} />}

      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-orange-100 rounded-xl flex items-center justify-center">
            <Bus size={20} className="text-[#F98C1F]" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-[#222222]">Fleet</h2>
            <p className="text-sm text-[#666666]">{visible.length} {showArchived ? 'archived' : 'active'}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-sm text-[#666666] cursor-pointer">
            <input type="checkbox" checked={showArchived} onChange={e => setShowArchived(e.target.checked)} className="rounded" />
            Show archived
          </label>
          <button onClick={openCreate}
            className="bg-[#F98C1F] hover:bg-[#F57C00] text-white px-4 py-2 rounded-xl font-semibold text-sm transition-colors">
            + Add Bus
          </button>
        </div>
      </div>

      {/* Table */}
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        {visible.length === 0 ? (
          <div className="text-center py-16 text-[#666666]">
            <Bus size={40} className="mx-auto mb-3 text-gray-300" />
            <p className="font-medium">No buses found</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-[#F5EDE3]">
              <tr>
                <th className="text-left px-4 py-3 font-semibold text-[#222222]">Bus #</th>
                <th className="text-left px-4 py-3 font-semibold text-[#222222] hidden md:table-cell">License Plate</th>
                <th className="text-left px-4 py-3 font-semibold text-[#222222] hidden lg:table-cell">Driver</th>
                <th className="text-left px-4 py-3 font-semibold text-[#222222] hidden lg:table-cell">Status</th>
                <th className="text-right px-4 py-3 font-semibold text-[#222222]">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {visible.map(bus => (
                <tr key={bus.id} className="hover:bg-orange-50/30 transition-colors">
                  <td className="px-4 py-3 font-medium text-[#222222]">
                    Bus #{bus.busNumber}
                    {bus.isArchived && (
                      <span className="ml-2 text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">archived</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-[#666666] hidden md:table-cell">{bus.licensePlate}</td>
                  <td className="px-4 py-3 text-[#666666] hidden lg:table-cell">{driverName(bus.driverId)}</td>
                  <td className="px-4 py-3 hidden lg:table-cell">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_CFG[bus.status] || STATUS_CFG.waiting}`}>
                      {bus.status || 'waiting'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(bus)}
                        className="p-1.5 hover:bg-orange-100 rounded-lg text-[#666666] hover:text-[#F98C1F] transition-colors" title="Edit">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => toggleArchive(bus)}
                        className="p-1.5 hover:bg-gray-100 rounded-lg text-[#666666] hover:text-[#222222] transition-colors"
                        title={bus.isArchived ? 'Restore' : 'Archive'}>
                        {bus.isArchived ? <RotateCcw size={15} /> : <Archive size={15} />}
                      </button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </div>

      {/* Modal */}
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-bold text-[#222222]">
                {editTarget ? 'Edit Bus' : 'Add Bus'}
              </h3>
              <button onClick={closeModal} className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-400">
                <X size={18} />
              </button>
            </div>
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <Field label="Bus Number *" value={form.busNumber} onChange={set('busNumber')} required placeholder="e.g. 24" />
                <Field label="License Plate *" value={form.licensePlate} onChange={set('licensePlate')} required placeholder="กข 1234" />
              </div>

              <Field label="School">
                <select value={form.schoolId} onChange={set('schoolId')}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#F98C1F]">
                  <option value="">Select school</option>
                  {activeSchools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </Field>

              <Field label="Assign Driver">
                <select value={form.driverId} onChange={set('driverId')}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#F98C1F]">
                  <option value="">No driver assigned</option>
                  {activeDrivers.map(d => <option key={d.id} value={d.id}>{d.name} — {d.licenseNumber || 'No license'}</option>)}
                </select>
              </Field>

              <div className="grid grid-cols-2 gap-3">
                <Field label="Current Lat" value={form.currentLat} onChange={set('currentLat')} placeholder="13.7563" />
                <Field label="Current Lng" value={form.currentLng} onChange={set('currentLng')} placeholder="100.5018" />
              </div>

              <div className="flex gap-3 pt-2">
                <button type="button" onClick={closeModal}
                  className="flex-1 border border-gray-200 rounded-xl py-2.5 text-[#666666] hover:bg-gray-50 text-sm font-medium">
                  Cancel
                </button>
                <button type="submit" disabled={saving}
                  className="flex-1 bg-[#F98C1F] hover:bg-[#F57C00] text-white rounded-xl py-2.5 font-semibold text-sm disabled:opacity-60">
                  {saving ? 'Saving...' : editTarget ? 'Save Changes' : 'Add Bus'}
                </button>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  )
}
